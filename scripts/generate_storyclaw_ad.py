...
('03_solution',PRODUCT+' 想法光点抵达真实 StoryClaw ClawBot，设备顶部白色标识和前面板接口保持一致；接触瞬间，一条蓝绿色路径从设备指向画面外的全球推理交换网络，强调这是一个承载任务接入与节点身份的入口，不表现硬件直接运行模型，不生成规格数字。优雅、惊喜、电影级。'),
('04_proof',PRODUCT+' 视觉化一次请求的可追踪旅程：金色光点从请求界面进入 StoryClaw，明确落到一个被选中的供应节点，再以一枚柔和的 receipt 回执回到创意画布；旁边只作背景的其它节点保持虚化，不表现请求串行经过多个节点。真实 ClawBot 外形、Logo和接口稳定，不出现收益、速度或虚构数字。'),
